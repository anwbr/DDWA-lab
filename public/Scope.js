...
DDWAApp.ScopesService = function() {
	...
	self.deleteScope = function(id)  {
        return new Promise((resolve, reject)=>{
            sendGetMethod('DELETE', '/' + id).then((data)=>{
                resolve(data);
            });
        });
	};
	...
	self.addWorker = (data)=>{
        return new Promise((resolve, reject)=>{
            sendPostMethod('POST', DDWAApp.CONSTANTS.SERVICE_URL, data).then(function (data) {
                resolve(data);
            });            
        });
	};

	self.updateWorker = (data, id)=>{
        return new Promise((resolve, reject)=>{
            sendPostMethod('PUT', DDWAApp.CONSTANTS.SERVICE_URL +"/"+id, data).then((data)=>{
               resolve(data);
            })
        });
	...
	self.getDetails = (id)=>{
        return new Promise((resolve, reject)=>{
            sendGetMethod('GET', '/' + id).then((data)=>{
                let scope = DDWAApp.Mappers.FactoryMapper.mapJSONToScope(data);
                resolve(scope);
            });
        });
    };
